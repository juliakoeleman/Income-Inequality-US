...
Allmerged <- Allmerged %>%
  select(State, State_ID, Year, everything()) %>%   
...
# Ensure Bachelor is numeric
Allmerged$Bachelor <- as.numeric(Allmerged$Bachelor)

# Create quartile-based education groups
Allmerged$Education <- cut(
  Allmerged$Bachelor,
  breaks = quantile(Allmerged$Bachelor, probs = seq(0, 1, by = 0.25), na.rm = TRUE),
  labels = c("Low", "Medium Low", "Medium High", "High"),
  include.lowest = TRUE
)

# Make sure Education is a factor with the right order
Allmerged$Education <- factor(
  Allmerged$Education,
...
ggplot(data = Allmerged, aes(x = Education, y = Top10_vs_bottom90)) +
  geom_boxplot() +
  labs(
